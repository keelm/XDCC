...
struct TreeParam : public dmlc::Parameter<TreeParam> {
  ...
  int num_feature;
  /*! \brief number of labels used for tree construction */
  int num_label;
  /*!
  ...
  TreeParam() {
    ...
    static_assert(sizeof(TreeParam) == (31 + 7) * sizeof(int),
  ...
  DMLC_DECLARE_PARAMETER(TreeParam) {
    ...
    DMLC_DECLARE_FIELD(num_label)
        .describe("Number of labels used in tree construction.");
...
class TreeModel {
 public:
  ...
  class Node {
   public:
    Node() : sindex_(0)/*, multilabel_leaf_vector(std::vector<double>(1,0.0))*/ {
      // assert compact alignment
    ...
    /*! \return get leaf vector of multilabel leaf node */
    inline std::vector<double> multilabel_leaf_value() const {
      return this->multilabel_leaf_vector;
    }
    /*! \return get leaf vector of multilabel leaf node as string */
    inline std::string multilabel_leaf_string() const{
      std::ostringstream oss;
      if (!this->multilabel_leaf_vector.empty())
      {
        std::copy(this->multilabel_leaf_vector.begin(), this->multilabel_leaf_vector.end()-1,
                  std::ostream_iterator<double>(oss, ","));
        oss << this->multilabel_leaf_vector.back();
      }
      return oss.str();
    }
    ...
    inline void set_leaf(bst_float value, int right = -1) {
      ...
      /*!
       * \brief set the leaf value of the multilabel node
       * \param vector of leaf values
       * \param right right index, could be used to store
       *        additional information
       */
      inline void set_multilabel_leaf(std::vector<double> leaf_vector, int right = -1) {
        this->multilabel_leaf_vector = leaf_vector;
        this->cleft_ = -1;
        this->cright_ = right;
      }
   ...
   private:
    ...
    std::vector<double> multilabel_leaf_vector;
...
struct RTreeNodeStat {
  ...
  /*! \brief weight of current node */
  std::vector<double> multilabel_base_weight;
  /*! \brief number of child that is leaf node known up to now */
  int leaf_child_cnt;

  inline std::string sum_all_hess_string() const{
      std::ostringstream oss;
      if (!this->sum_all_hess.empty())
      {
          std::copy(this->sum_all_hess.begin(), this->sum_all_hess.end()-1,
                    std::ostream_iterator<int>(oss, ","));
          oss << this->sum_all_hess.back();
      }
      return oss.str();
  }
};
...
class RegTree: public TreeModel<bst_float, RTreeNodeStat> {
 public:
  ...
  /*!
   * \brief get the prediction vector of regression multilabel tree, only accepts dense feature vector
   * \param feat dense feature vector, if the feature is missing the field is set to NaN
   * \param root_id starting root index of the instance
   * \return the leaf index of the given feature
   */
  inline std::vector<double> MultilabelPredict(const RegTree::FVec& feat, unsigned root_id) const;
  /*!
...
inline std::vector<double> RegTree::MultilabelPredict(const RegTree::FVec& feat, unsigned root_id) const {
  int pid = this->GetLeafIndex(feat, root_id);
  return (*this)[pid].multilabel_leaf_value();
}
