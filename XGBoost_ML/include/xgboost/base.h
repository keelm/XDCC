...
class bst_gpair_internal {
  ...
  T hess_;
  /*! \brief gradient statistics vector*/
  std::vector<double> grad_vec_;
  /*! \brief second order gradient statistics vector*/
  std::vector<double> hess_vec_;

  XGBOOST_DEVICE void SetGrad(float g) { grad_ = g; }
  XGBOOST_DEVICE void SetHess(float h) { hess_ = h; }
  XGBOOST_DEVICE void SetMultilabelGrad(std::vector<double>& g) { grad_vec_ = g; }
  XGBOOST_DEVICE void SetMultilabelHess(std::vector<double>& h) { hess_vec_ = h; }

 public:
  typedef T value_t;

  XGBOOST_DEVICE bst_gpair_internal() : grad_(0), hess_(0), grad_vec_(std::vector<double>(1, 0.0)), hess_vec_(std::vector<double>(1, 0.0)) {}

 // XGBOOST_DEVICE bst_gpair_internal(int nlabel) : grad_(0), hess_(0), grad_vec_(std::vector<double>(nlabel, 0.0)), hess_vec_(std::vector<double>(nlabel, 0.0)) {}
  ...
  XGBOOST_DEVICE bst_gpair_internal(std::vector<double>& grad_v, std::vector<double>& hess_v) {
    SetMultilabelGrad(grad_v);
    SetMultilabelHess(hess_v);
  }

  // Copy constructor if of same value type
  XGBOOST_DEVICE bst_gpair_internal(const bst_gpair_internal<T> &g)
      : grad_(g.grad_), hess_(g.hess_), grad_vec_(g.grad_vec_), hess_vec_(g.hess_vec_) {}
  ...
  XGBOOST_DEVICE bst_gpair_internal(const bst_gpair_internal<T2> &g) {
    SetGrad(g.GetGrad());
    SetHess(g.GetHess());
    SetMultilabelGrad(g.GetMultilabelGrad());
    SetMultilabelHess(g.GetMultilabelHess());
  }

  XGBOOST_DEVICE float GetGrad() const { return grad_; }
  XGBOOST_DEVICE float GetHess() const { return hess_; }
  XGBOOST_DEVICE std::vector<double> GetMultilabelGrad() const { return grad_vec_; }
  XGBOOST_DEVICE std::vector<double> GetMultilabelHess() const { return hess_vec_; }

  XGBOOST_DEVICE bst_gpair_internal<T> &operator+=(
      const bst_gpair_internal<T> &rhs) {
    grad_ += rhs.grad_;
    hess_ += rhs.hess_;
    std::transform(grad_vec_.begin(), grad_vec_.end(), rhs.grad_vec_.begin(), grad_vec_.begin(), std::plus<double>());
    std::transform(hess_vec_.begin(), hess_vec_.end(), rhs.hess_vec_.begin(), hess_vec_.begin(), std::plus<double>());
  ...
  XGBOOST_DEVICE bst_gpair_internal<T> operator+(
    ...
    std::transform(grad_vec_.begin(), grad_vec_.end(), rhs.grad_vec_.begin(), g.grad_vec_.begin(), std::plus<double>());
    std::transform(hess_vec_.begin(), hess_vec_.end(), rhs.hess_vec_.begin(), g.hess_vec_.begin(), std::plus<double>());
    return g;
  }
  ...
  XGBOOST_DEVICE bst_gpair_internal<T> &operator-=(
    ...
    grad_ -= rhs.grad_;
    hess_ -= rhs.hess_;
    std::transform(grad_vec_.begin(), grad_vec_.end(), rhs.grad_vec_.begin(), grad_vec_.begin(), std::minus<double>());
    std::transform(hess_vec_.begin(), hess_vec_.end(), rhs.hess_vec_.begin(), hess_vec_.begin(), std::minus<double>());
    return *this;
  }
  ...
  XGBOOST_DEVICE bst_gpair_internal<T> operator-(
    ...
    g.grad_ = grad_ - rhs.grad_;
    g.hess_ = hess_ - rhs.hess_;
    std::transform(grad_vec_.begin(), grad_vec_.end(), rhs.grad_vec_.begin(), g.grad_vec_.begin(), std::minus<double>());
    std::transform(hess_vec_.begin(), hess_vec_.end(), rhs.hess_vec_.begin(), g.hess_vec_.begin(), std::minus<double>());
    return g;
  }
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    //TODO: add output for grad and hess vector
    // os << g.GetGrad() << "/" << g.GetHess() << " GradVec: " << grad_vec_ << " HessVec: " << hess_vec_;
...
template<>
inline XGBOOST_DEVICE std::vector<double> bst_gpair_internal<int64_t>::GetMultilabelGrad() const {
    return grad_vec_;
}
template<>
inline XGBOOST_DEVICE std::vector<double> bst_gpair_internal<int64_t>::GetMultilabelHess() const {
    return hess_vec_;
}
template<>
