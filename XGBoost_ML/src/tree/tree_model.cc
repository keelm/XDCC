  ...
  if (tree[nid].is_leaf()) {
    if (format == "json") {
      fo << "{ \"nodeid\": " << nid
         ...
         << ", \"leafVec\": " << tree[nid].multilabel_leaf_string();
      if (with_stats) {
        fo << ", \"cover\": " << tree.stat(nid).sum_hess
           << ", \"coverVec\": " << tree.stat(nid).sum_all_hess_string();
      }
      fo << " }";
    } else {
      fo << nid << ":leaf=" << tree[nid].leaf_value() << "; leafVec=" << tree[nid].multilabel_leaf_string();
      if (with_stats) {
        fo << ",cover=" << tree.stat(nid).sum_hess
           << ",coverVec=" << tree.stat(nid).sum_all_hess_string();
      }
