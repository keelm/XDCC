...
struct TrainParam : public dmlc::Parameter<TrainParam> {
  ...
  int n_gpus;
  // split method for multilabel trees
  int ml_split_method;
  ...
  DMLC_DECLARE_PARAMETER(TrainParam) {
    ...
    DMLC_DECLARE_FIELD(ml_split_method)
        .set_default(1)
        .describe("Method used for split calculation of multilabel trees");
...
// calculate the cost of loss function
template <typename TrainingParams, typename T>
XGBOOST_DEVICE inline T CalcMultilabelGain(const TrainingParams &p, const std::vector<T> &sum_all_grad,
                                           const std::vector<T> &sum_all_hess) {
    T bestTemp = 0;

    //std::vector<T> temp = std::vector<T>(sum_all_grad.size());
    //std::transform(sum_all_hess.begin(), sum_all_hess.end(), temp.begin(), bind2nd(std::plus<double>(), p.reg_lambda));
    //std::transform(sum_all_grad.begin(), sum_all_grad.end(), temp.begin(), temp.begin(), std::divides<double>());
    //std::transform(temp.begin(), temp.end(), temp.begin(), std::bind1st(std::multiplies<double>(), -1));
    //bestTemp = std::accumulate(temp.begin(), temp.end(), 0);
    //return bestTemp;

    if(p.ml_split_method == 1 || p.ml_split_method == 2 || p.ml_split_method == 5)
        bestTemp = -1000000;
    if (p.reg_alpha == 0.0f) {
        for(unsigned int i = 0; i<sum_all_grad.size(); i++){
            if(p.ml_split_method == 1){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp = std::max(T(0.0), bestTemp);
                } else {
                    bestTemp = std::max((Sqr(sum_all_grad[i]) / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 2){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp = std::max(T(0.0), bestTemp);
                } else {
                    bestTemp = std::max((-sum_all_grad[i] / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 3){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += (Sqr(sum_all_grad[i]) / (sum_all_hess[i] + p.reg_lambda));
                }
            } else if(p.ml_split_method == 4){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += (-(sum_all_grad[i]) / (sum_all_hess[i] + p.reg_lambda));
                }
            } else if(p.ml_split_method == 5){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp = std::max(T(0.0), bestTemp);
                } else {
                    bestTemp = std::max(std::abs(-(sum_all_grad[i]) / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 6){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += std::abs(-(sum_all_grad[i]) / (sum_all_hess[i] + p.reg_lambda));
                }
            }
        }
    } else {
        for(unsigned int i = 0; i<sum_all_grad.size(); i++){
            if(p.ml_split_method == 1){
                if(sum_all_hess[i] < p.min_child_weight){
                    ...
                    bestTemp = std::max((Sqr(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 2){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp = std::max(T(0.0), bestTemp);
                } else {
                    bestTemp = std::max((-(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 3){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += (Sqr(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda));
                }
            } else if(p.ml_split_method == 4){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += (-(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda));
                }
            } else if(p.ml_split_method == 5){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp = std::max(T(0.0), bestTemp);
                } else {
                    bestTemp = std::max(std::abs(-(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda)), bestTemp);
                }
            } else if(p.ml_split_method == 6){
                if(sum_all_hess[i] < p.min_child_weight){
                    bestTemp += T(0.0);
                } else {
                    bestTemp += std::abs(-(ThresholdL1(sum_all_grad[i], p.reg_alpha)) / (sum_all_hess[i] + p.reg_lambda));
                }
            }
        }
    }
    if (p.max_delta_step != 0.0f) {
        if (bestTemp > p.max_delta_step)
            bestTemp = p.max_delta_step;
        if (bestTemp < -p.max_delta_step)
            bestTemp = -p.max_delta_step;
    }
    return bestTemp;
...
// calculate the cost of loss function
template <typename TrainingParams, typename T>
XGBOOST_DEVICE inline T CalcMultilabelGain(const TrainingParams &p, std::vector<T> sum_grad_left,
                                           std::vector<T> sum_hess_left, std::vector<T> sum_grad_right,
                                           std::vector<T> sum_hess_right) {
    T bestTemp = -100000000;
    T left_score = 0;
    T right_score = 0;
    if (p.reg_alpha == 0.0f) {
        for(unsigned int i = 0; i<sum_grad_left.size(); i++){
            left_score += std::abs(Sqr(sum_grad_left[i]) / (sum_hess_left[i] + p.reg_lambda));
            right_score += std::abs(Sqr(sum_grad_right[i]) / (sum_hess_right[i] + p.reg_lambda));
        }
        bestTemp = left_score + right_score;
    } else {
        for(unsigned int i = 0; i<sum_grad_left.size(); i++){
            left_score += std::abs(Sqr(ThresholdL1(sum_grad_left[i], p.reg_alpha)) / (sum_hess_left[i] + p.reg_lambda));
            right_score += std::abs(Sqr(ThresholdL1(sum_grad_right[i], p.reg_alpha)) / (sum_hess_right[i] + p.reg_lambda));
        }
        bestTemp = left_score + right_score;
    }
    if (p.max_delta_step != 0.0f) {
...
// calculate weight given the statistics
template <typename TrainingParams, typename T>
XGBOOST_DEVICE inline std::vector<T> CalcMultilabelWeights(const TrainingParams &p, const std::vector<T> &sum_all_grad, const std::vector<T> &sum_all_hess) {
    std::vector<T> w = std::vector<T>(sum_all_grad.size());
    for(unsigned int i = 0; i<sum_all_grad.size(); i++){
        if (sum_all_grad[i] < p.min_child_weight){
            w[i] = 0;
        }
        if (p.reg_alpha == 0.0f) {
            w[i] = -sum_all_grad[i] / (sum_all_hess[i] + p.reg_lambda);
        } else {
            w[i] = -ThresholdL1(sum_all_grad[i], p.reg_alpha) / (sum_all_hess[i] + p.reg_lambda);
        }
        if (p.max_delta_step != 0.0f) {
            if (w[i] > p.max_delta_step)
                w[i] = p.max_delta_step;
            if (w[i] < -p.max_delta_step)
                w[i] = -p.max_delta_step;
        }
    }
...
/*! \brief core statistics used for tree construction */
struct XGBOOST_ALIGNAS(16) MultilabelGradStats {
    /*! \brief sum gradient statistics */
    std::vector<double> sum_all_grad;
    /*! \brief sum hessian statistics */
    std::vector<double> sum_all_hess;
    /*! \brief smallest hess value*/
    double min_hess;
    ...
    static const int kSimpleStats = 1;
    /*! \brief constructor, the object must be cleared during construction */
    explicit MultilabelGradStats(const TrainParam& param) {
        num_label = param.size_leaf_vector;
        sum_all_grad.resize(param.size_leaf_vector);
        sum_all_hess.resize(param.size_leaf_vector);
        this->Clear();
    }

    template <typename gpair_t>
    XGBOOST_DEVICE explicit MultilabelGradStats(const gpair_t &sum)
    : sum_all_grad(sum.GetMultilabelGrad()), sum_all_hess(sum.GetMultilabelHess()) {}
    /*! \brief clear the statistics */
    inline void Clear() {
        std::fill(sum_all_grad.begin(), sum_all_grad.end(), 0.0);
        std::fill(sum_all_hess.begin(), sum_all_hess.end(), 0.0);
        min_hess = 0;
    }
    /*! \brief check if necessary information is ready */
    inline static void CheckInfo(const MetaInfo& info) {}
     ...
     * \param p the gradient pair
     */
    inline void Add(bst_gpair& p) { this->Add(p.GetMultilabelGrad(), p.GetMultilabelHess()); }
    /*!
     * \brief accumulate statistics, more complicated version
     * \param gpair the vector storing the gradient statistics
     * \param info the additional information
    ...
    inline void Add(const std::vector<bst_gpair>& gpair, const MetaInfo& info,
                    bst_uint ridx) {
        const bst_gpair& b = gpair[ridx];
        this->Add(b.GetMultilabelGrad(), b.GetMultilabelHess());
    }
    /*! \brief calculate leaf weight */
    template <typename param_t>
    XGBOOST_DEVICE inline std::vector<double> CalcMultilabelWeights(const param_t &param) const {
        return xgboost::tree::CalcMultilabelWeights(param, sum_all_grad, sum_all_hess);
    }
    /*! \brief calculate gain of the solution */
    template <typename param_t>
    inline double CalcGain(const param_t& param) const {
        return xgboost::tree::CalcMultilabelGain(param, sum_all_grad, sum_all_hess);
    }
    /*! \brief add statistics to the data */
    inline void Add(const MultilabelGradStats& b) {
        //for(unsigned int i = 0; i<sum_all_grad.size(); i++){
        //    sum_all_grad[i] = sum_all_grad[i] + b.sum_all_grad[i];
        //    sum_all_hess[i] = sum_all_hess[i] + b.sum_all_hess[i];
        //}
        std::transform(sum_all_grad.begin(), sum_all_grad.end(), b.sum_all_grad.begin(), sum_all_grad.begin(), std::plus<double>());
        std::transform(sum_all_hess.begin(), sum_all_hess.end(), b.sum_all_hess.begin(), sum_all_hess.begin(), std::plus<double>());
        min_hess = *std::min_element(sum_all_hess.begin(), sum_all_hess.end());
    }
    /*! \brief same as add, reduce is used in All Reduce */
    inline static void Reduce(MultilabelGradStats& a, const MultilabelGradStats& b) { // NOLINT(*)
        a.Add(b);
    }
    /*! \brief set current value to a - b */
    inline void SetSubstract(const MultilabelGradStats& a, const MultilabelGradStats& b) {
        for(unsigned int i = 0; i<sum_all_grad.size(); i++) {
            sum_all_grad[i] = a.sum_all_grad[i] - b.sum_all_grad[i];
            sum_all_hess[i] = a.sum_all_hess[i] - b.sum_all_hess[i];
        }
        //std::transform(a.sum_all_grad.begin(), a.sum_all_grad.end(), b.sum_all_grad.begin(), sum_all_grad.begin(), std::minus<double>());
        //std::transform(a.sum_all_hess.begin(), a.sum_all_hess.end(), b.sum_all_hess.begin(), sum_all_hess.begin(), std::minus<double>());
        min_hess = *std::min_element(sum_all_hess.begin(), sum_all_hess.end());
    }
    /*! \return whether the statistics is not used yet */
    inline bool Empty() const {
        //double sum_of_elems = 0;
        //for (auto& n : sum_all_hess)
        //    sum_of_elems += n;
        double sum_of_elems = std::accumulate(sum_all_hess.begin(), sum_all_hess.end(), 0);
        return sum_of_elems == 0.0;
    }
    /*! \brief set leaf vector value based on statistics */
    inline void SetLeafVec(const TrainParam& param, bst_float* vec) const {
    }
    ...
    MultilabelGradStats() {}
    /*! \brief add statistics to the data */
    inline void Add(const std::vector<double> &grad, const std::vector<double> &hess) {
        for(unsigned int i = 0; i<grad.size(); i++){
            sum_all_grad[i] = sum_all_grad[i] + grad[i];
            sum_all_hess[i] = sum_all_hess[i] + hess[i];
        }
        //std::transform(sum_all_grad.begin(), sum_all_grad.end(), grad.begin(), sum_all_grad.begin(), std::plus<double>());
        //std::transform(sum_all_hess.begin(), sum_all_hess.end(), hess.begin(), sum_all_hess.begin(), std::plus<double>());
        min_hess = *std::min_element(sum_all_hess.begin(), sum_all_hess.end());
    }
};
...
struct MultilabelNoConstraint {
    inline static void Init(TrainParam *param, unsigned num_feature) {
        param->monotone_constraints.resize(num_feature, 0);
    }
    inline double CalcSplitGain(const TrainParam &param, int constraint,
                                MultilabelGradStats left, MultilabelGradStats right) const {
        return left.CalcGain(param) + right.CalcGain(param);
    }
    inline std::vector<double> CalcMultilabelWeights(const TrainParam &param, MultilabelGradStats stats) const {
        return stats.CalcMultilabelWeights(param);
    }
    inline double CalcGain(const TrainParam &param, MultilabelGradStats stats) const {
        return stats.CalcGain(param);
    }
    inline void SetChild(const TrainParam &param, bst_uint split_index,
                         MultilabelGradStats left, MultilabelGradStats right, MultilabelNoConstraint *cleft,
                         MultilabelNoConstraint *cright) {
    }
};
...
struct MultilabelValueConstraint {
    std::vector<double> lower_bound;
    std::vector<double> upper_bound;
    XGBOOST_DEVICE MultilabelValueConstraint()
            : lower_bound(std::vector<double>(1,-std::numeric_limits<double>::max())),
              upper_bound(std::vector<double>(1,std::numeric_limits<double>::max())) {}
    inline static void Init(TrainParam *param, unsigned num_feature) {
        param->monotone_constraints.resize(num_feature, 0);
    }
    template <typename param_t>
    XGBOOST_DEVICE inline std::vector<double> CalcMultilabelWeights(const param_t &param, MultilabelGradStats stats) const {
        std::vector<double> w = stats.CalcMultilabelWeights(param);
        for(std::vector<int>::size_type i = 0; i != w.size(); i++) {
            if (w[i] < lower_bound[i]) {
                w[i] = lower_bound[i];
            } else if (w[i] > upper_bound[i]) {
                w[i] = upper_bound[i];
            }
        }
        return w;
    ...
    template <typename param_t>
    XGBOOST_DEVICE inline double CalcGain(const param_t &param, MultilabelGradStats stats) const {
        return 0.0;
        //return CalcGainGivenWeight(param, stats.sum_grad, stats.sum_hess,
        //                           CalcMultilabelWeights(param, stats));
    }
    ...
    XGBOOST_DEVICE inline double CalcSplitGain(const param_t &param, int constraint,
                                               MultilabelGradStats left, MultilabelGradStats right) const {
        const double negative_infinity = -std::numeric_limits<double>::infinity();
        std::vector<double> wleft = CalcMultilabelWeights(param, left);
        std::vector<double> wright = CalcMultilabelWeights(param, right);
        /*double gain =
                CalcGainGivenWeight(param, left.sum_grad, left.sum_hess, wleft) +
                CalcGainGivenWeight(param, right.sum_grad, right.sum_hess, wright);
        if (constraint == 0) {
    ...
    inline void SetChild(const TrainParam &param, bst_uint split_index,
                         MultilabelGradStats left, MultilabelGradStats right, MultilabelValueConstraint *cleft,
                         MultilabelValueConstraint *cright) {
        int c = param.monotone_constraints.at(split_index);
        *cleft = *this;
        *cright = *this;
        if (c == 0)
        ...
        std::vector<double> wleft = CalcMultilabelWeights(param, left);
        std::vector<double> wright = CalcMultilabelWeights(param, right);
        std::vector<double> mid(param.size_leaf_vector, 0.0);
        std::transform(wleft.begin(), wleft.end(), wright.begin(), mid.begin(), std::plus<double>());
        std::transform(mid.begin(), mid.end(), mid.begin(), std::bind1st(std::multiplies<double>(), 0.5));
        //CHECK(!std::isnan(mid));
        if (c < 0) {
            cleft->lower_bound = mid;
            cright->upper_bound = mid;
