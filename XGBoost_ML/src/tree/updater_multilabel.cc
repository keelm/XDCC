...
DMLC_REGISTRY_FILE_TAG(updater_multilabel);

/*! \brief column-wise update to construct a tree */
template<typename TStats, typename TConstraint>
class MultilabelColMaker: public TreeUpdater {
 public:
 ...
 protected:
  ...
  struct NodeEntry {
    ...
    explicit NodeEntry(const TrainParam& param)
        : stats(param), root_gain(0.0f), weights({param.size_leaf_vector, 0.0}){
    }
  };
   ...
   public:
    ...
    virtual void Update(const std::vector<bst_gpair>& gpair,
      ...
      for (size_t i = 0; i < qexpand_.size(); ++i) {
        ...
        std::vector<double> leaf_vec(param.size_leaf_vector, 0.0);
        std::transform(snode[nid].weights.begin(), snode[nid].weights.end(), leaf_vec.begin(), std::bind1st(std::multiplies<double>(), param.learning_rate));
        (*p_tree)[nid].set_multilabel_leaf(leaf_vec);
      }
      // remember auxiliary statistics in the tree node
      for (int nid = 0; nid < p_tree->param.num_nodes; ++nid) {
        p_tree->stat(nid).loss_chg = snode[nid].best.loss_chg;
        p_tree->stat(nid).multilabel_base_weight = snode[nid].weights;
        p_tree->stat(nid).sum_all_hess = snode[nid].stats.sum_all_hess;
   ...
   protected:
    ...
    inline void InitData(const std::vector<bst_gpair>& gpair,
      ...
      CHECK_EQ(tree.param.num_nodes, tree.param.num_roots)
          << "MultilabelColMaker: can only grow new tree";
    ...
    inline void InitNewNode(const std::vector<int>& qexpand,
      ...
      for (size_t j = 0; j < qexpand.size(); ++j) {
        ...
        snode[nid].weights = constraints_[nid].CalcMultilabelWeights(param, snode[nid].stats);
      }
    }
      ...
      for (bst_omp_uint j = 0; j < nnode; ++j) {
        ...
        for (int tid = 0; tid < this->nthread; ++tid) {
          ...
          if (need_forward && tid != 0) {
            ...
            if (c.min_hess >= param.min_child_weight &&
                e.stats.min_hess >= param.min_child_weight) {
          ...
          if (need_backward) {
            ...
            if (c.min_hess >= param.min_child_weight &&
                tmp.min_hess >= param.min_child_weight) {
              bst_float loss_chg = static_cast<bst_float>(
                  constraints_[nid].CalcSplitGain(
          ...
          if (e.stats.Empty()) {
            ...
            if (fvalue != e.first_fvalue) {
              ...
              if (need_backward) {
                ...
                if (c.min_hess >= param.min_child_weight &&
                    cright.min_hess >= param.min_child_weight) {
                  bst_float loss_chg = static_cast<bst_float>(
                      constraints_[nid].CalcSplitGain(
    ...
    inline void EnumerateSplitCacheOpt(const ColBatch::Entry *begin,
      ...
      for (size_t i = 0; i < qexpand.size(); ++i) {
        ...
        ThreadEntry &e = temp[nid];
        c.SetSubstract(snode[nid].stats, e.stats);
        if (e.stats.min_hess >= param.min_child_weight &&
            c.min_hess >= param.min_child_weight) {
          bst_float loss_chg;
          if (d_step == -1) {
            loss_chg = static_cast<bst_float>(
                constraints_[nid].CalcSplitGain(
    ...
    inline void FindSplit(int depth,
      ...
      for (size_t i = 0; i < qexpand.size(); ++i) {
        ...
        if (e.best.loss_chg > rt_eps) {
          ...
          (*p_tree)[(*p_tree)[nid].cleft()].set_multilabel_leaf(std::vector<double>(param.size_leaf_vector, 0.0), 0);
          (*p_tree)[(*p_tree)[nid].cright()].set_multilabel_leaf(std::vector<double>(param.size_leaf_vector, 0.0), 0);
        } else {
          std::vector<double> leaf_vec(param.size_leaf_vector, 0.0);
          std::transform(e.weights.begin(), e.weights.end(), leaf_vec.begin(), std::bind1st(std::multiplies<double>(), param.learning_rate));
          (*p_tree)[nid].set_multilabel_leaf(leaf_vec);
          //(*p_tree)[nid].set_multilabel_leaf(std::transform(e.weights.begin(), e.weights.end(), e.weights.begin(), std::multiplies<double>(), param.learning_rate));
          //(*p_tree)[nid].set_multilabel_leaf(std::accumulate(std::begin(e.weights), std::end(e.weights), param.learning_rate, std::multiplies<double>()));
        }
      }
    }
 ...
 public:
  ...
  void Init(const std::vector<std::pair<std::string, std::string> >& args) override {
    ...
    if (inner_.get() == nullptr) {
      //if (monotone_) {
      if(false){
        inner_.reset(new MultilabelColMaker<MultilabelGradStats, MultilabelValueConstraint>());
      } else {
        inner_.reset(new MultilabelColMaker<MultilabelGradStats, MultilabelNoConstraint>());
      }
    }
...
XGBOOST_REGISTER_TREE_UPDATER(MultilabelColMaker, "grow_multilabel")
.describe("Grow multilabel tree with parallelization over columns.")
.set_body([]() {
