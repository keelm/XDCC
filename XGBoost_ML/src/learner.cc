...
struct LearnerModelParam : public dmlc::Parameter<LearnerModelParam> {
  ...
  int num_class;
  /* \brief number of labels, if it is multilabel classification  */
  int num_label;
  ...
  DMLC_DECLARE_PARAMETER(LearnerModelParam) {
    ...
    DMLC_DECLARE_FIELD(num_label).set_default(0).set_lower_bound(0).describe(
    "Number of label option for multilabel classifier. "
    " By default equals 0 and corresponds to binary classifier.");
  }
};
...
class LearnerImpl : public Learner {
 public:
  ...
  void Configure(
    ...
    if (cfg_.count("num_label") != 0) {
      cfg_["size_leaf_vector"] = cfg_["num_label"];
      if (atoi(cfg_["num_label"].c_str()) > 1 && cfg_.count("objective") == 0) {
        cfg_["objective"] = "multilabel";
      }
    }
    ...
    cfg_["num_label"] = common::ToString(mparam.num_label);
