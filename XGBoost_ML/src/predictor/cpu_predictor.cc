  ...
  static std::vector<double> PredMultilabelVector(const RowBatch::Inst& inst,
    ...
    for (size_t i = tree_begin; i < tree_end; ++i) {
      if (tree_info[i] == bst_group) {
        ...
        //LOG(CONSOLE)<<tree_pred[0] << " " << tree_pred[1] << " " << tree_pred[2];
...
#pragma omp parallel for schedule(static)
        ...
        for (int k = 0; k < K; ++k) {
          if (model.param.size_leaf_vector > 0){
            ...
            for (int gid = 0; gid < num_group; ++gid) {
              ...
              preds[offset] += this->PredValue(inst[k], model.trees, model.tree_info, gid,
  ...
  void InitOutPredictions(const MetaInfo& info,
    ...
    size_t n;
    if (model.param.size_leaf_vector > 0){
      n = info.num_row * model.param.size_leaf_vector;
    } else {
      n = model.param.num_output_group * info.num_row;
    }
