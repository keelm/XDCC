...
class Dart : public GBTree {
 public:
  ...
  void PredictInstance(const SparseBatch::Inst& inst,
    ...
    if(model_.param.size_leaf_vector > 0){
      out_preds->resize(model_.param.size_leaf_vector);
    } else {
    ...
    if (model_.param.num_label > 0) {
      std::vector<double> pred_vector = PredMultilabelValue(inst, 0, root_index,
                                                             &thread_temp[0], 0, ntree_limit);
      for (int gid = 0; gid < model_.param.size_leaf_vector; ++gid) {
        (*out_preds)[gid] = pred_vector[gid] + model_.base_margin;
      }
    } else {
      for (int gid = 0; gid < model_.param.num_output_group; ++gid) {
        (*out_preds)[gid]
 ...
 protected:
  ...
  inline void PredLoopInternal(
    ...
    if (init_out_preds) {
      size_t n;
      if (model_.param.size_leaf_vector > 0){
        n = p_fmat->info().num_row * model_.param.size_leaf_vector;
      } else {        
        n = num_group * p_fmat->info().num_row;
      }
  ...
  inline void PredLoopSpecalize(
    ...
    CHECK_EQ(preds.size(), p_fmat->info().num_row * num_group * model_.param.size_leaf_vector);
    ...
    while (iter->Next()) {
      ...
      for (bst_omp_uint i = 0; i < nsize - rest; i += K) {
        ...
        for (int k = 0; k < K; ++k) {
          if (model_.param.num_label > 0) {
            std::vector<double> pred_vector = PredMultilabelValue(inst[k], 0, info.GetRoot(ridx[k]),
                                                                   &feats, tree_begin, tree_end);
            for (int gid = 0; gid < model_.param.size_leaf_vector; ++gid) {
              const size_t offset = ridx[k] * model_.param.size_leaf_vector + gid;
      ...
      for (bst_omp_uint i = nsize - rest; i < nsize; ++i) {
        ...
        if (model_.param.num_label > 0) {
          std::vector<double> pred_vector = PredMultilabelValue(inst, 0, info.GetRoot(ridx),
                                                                 &feats, tree_begin, tree_end);
          for (int gid = 0; gid < model_.param.size_leaf_vector; ++gid) {
            const size_t offset = ridx * model_.param.size_leaf_vector + gid;
            preds[offset] += pred_vector[gid];
          }
        } else {
          for (int gid = 0; gid < num_group; ++gid) {
  ...
  // predict the leaf scores without dropped trees
  inline std::vector<double> PredMultilabelValue(const RowBatch::Inst &inst,
                             int bst_group,
                             unsigned root_index,
                             RegTree::FVec *p_feats,
                             unsigned tree_begin,
                             unsigned tree_end) {
    std::vector<double> psum = std::vector<double>(model_.param.num_label, 0.0);
    p_feats->Fill(inst);
    for (size_t i = tree_begin; i < tree_end; ++i) {
      if (model_.tree_info[i] == bst_group) {
        bool drop = (std::binary_search(idx_drop.begin(), idx_drop.end(), i));
        if (!drop) {
          int tid = model_.trees[i]->GetLeafIndex(*p_feats, root_index);
          std::vector<double> tree_pred = (*model_.trees[i])[tid].multilabel_leaf_value();
